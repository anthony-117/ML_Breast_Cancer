...
plot_confusion_matrix <- function(cm_obj) {
  ...
  # Create a heatmap
  plot <- ggplot(cm_df, aes(x = Actual, y = Predicted, fill = Frequency)) +
    geom_tile(color = "white") +
    geom_text(aes(label = Frequency), size = 6, color = "white") +
...
NN.train <- function(NN, X, Y, epochs, learning_rate = 0.01, 
  ...
  # Initialize vectors for accuracy
  train_accuracies <- numeric(epochs)
  test_accuracies <- numeric(epochs)
  ...
  for (epoch in 1:epochs) {
    ...
    # Calculate accuracy
    train_pred_class <- ifelse(NN.predict(NN, X.train_full) > 0.5, 1, 0)
    test_pred_class <- ifelse(test_output > 0.5, 1, 0)
    
    train_accuracy <- mean(train_pred_class == Y.train_full)
    test_accuracy <- mean(test_pred_class == Y.test)
    ...
    if (epoch %% 10 == 0) { 
      train_predictions[[length(train_predictions) + 1]] <- train_pred_class
      train_true_values[[length(train_true_values) + 1]] <- Y.train_full
      X_train_stored[[length(X_train_stored) + 1]] <- X.train_full
      
      test_predictions[[length(test_predictions) + 1]] <- test_pred_class
      test_true_values[[length(test_true_values) + 1]] <- Y.test
      X_test_stored[[length(X_test_stored) + 1]] <- X.test  
    }
    
    if (verbose && epoch %% 100 == 0) {
      cat(sprintf("Epoch %d | Train Cost: %.6f | Test Cost: %.6f | Train Accuracy: %.4f | Test Accuracy: %.4f\n",
                  epoch, epoch_cost, test_cost, train_accuracy, test_accuracy))
      
      # Plot confusion matrix for test set using the custom function
      cm <- confusionMatrix(factor(test_pred_class), factor(Y.test))
  ...
  return(list(
    ...
    test_accuracy = mean(test_accuracies)
  ))
}
